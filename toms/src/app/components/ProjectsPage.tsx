...
import { Search, Plus, Clock, CheckCircle2, Users, Calendar as CalendarIcon, Link2 } from 'lucide-react';
...
export function ProjectsPage() {
  ...
  return (
    <div className="flex flex-col h-full bg-[#f5f5f5] dark:bg-[#1a1a1a]">
      {/* View Tabs Bar */}
      <div className="bg-white dark:bg-[#1e1e1e] border-b border-[rgba(0,0,0,0.05)] dark:border-[rgba(255,255,255,0.1)] px-8 py-4">
      ...
      {/* Content Area */}
      <div className={`flex-1 overflow-auto ${activeView === 'roadmap' ? 'p-0' : 'p-8'}`}>
  ...
  return (
    <button
      ...
      className={`px-4 py-2 text-xs font-medium rounded transition-colors ${
        isActive
          ? 'bg-white dark:bg-[#2a2a2a] text-[#111] dark:text-[#e5e5e5] border border-[rgba(0,0,0,0.1)] dark:border-[rgba(255,255,255,0.1)]'
          ...
          {events.map((event, index) => (
            <div key={event.id} className="relative flex gap-6">
              ...
              <div className="relative z-10 flex-shrink-0 mt-1">
                <div
                  className="size-4 rounded-full border-4 border-white dark:border-[#1a1a1a] shadow-sm"
                ...
                <div className="bg-white dark:bg-[#1e1e1e] rounded-lg border border-[rgba(0,0,0,0.1)] dark:border-[rgba(255,255,255,0.1)] p-6 hover:shadow-md transition-shadow">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    {event.status === 'completed' && (
                      <span className="px-3 py-1 bg-[#d4f4dd] text-[#0d7c2e] text-[10px] font-medium rounded">
                        Completed
                      </span>
                    )}
                    {event.status === 'in-progress' && (
                      <span className="px-3 py-1 bg-[#d4e7ff] text-[#0066cc] text-[10px] font-medium rounded">
                        In Progress
                      </span>
                    )}
                  </div>
                  {event.assignees && (
                    <div className="flex items-center gap-2 mt-4">
                      <Users className="size-3.5 text-[#999] dark:text-[#888]" />
                      <span className="text-[10px] text-[#999] dark:text-[#888]">{event.assignees}</span>
                    </div>
                  )}
                </div>
              </div>
    ...
    <div className="bg-white dark:bg-[#1e1e1e] rounded-lg border border-[rgba(0,0,0,0.08)] dark:border-[rgba(255,255,255,0.1)] overflow-hidden">
      ...
      <div className="grid grid-cols-[280px_1fr] border-b border-[rgba(0,0,0,0.08)] dark:border-[rgba(255,255,255,0.1)]">
        <div className="px-6 py-3 bg-[#fafafa] dark:bg-[#2a2a2a] border-r border-[rgba(0,0,0,0.08)] dark:border-[rgba(255,255,255,0.1)]">
          <span className="text-[10px] font-medium text-[#999] dark:text-[#888] uppercase tracking-wider">TASK</span>
        </div>
        <div className="grid grid-cols-4 bg-[#fafafa] dark:bg-[#2a2a2a]">
          {weeks.map((week, index) => (
            <div key={week} className="px-4 py-3 text-center border-l border-[rgba(0,0,0,0.08)] dark:border-[rgba(255,255,255,0.1)] first:border-l-0">
              <span className="text-[10px] font-medium text-[#999] dark:text-[#888]">{week}</span>
            </div>
          ))}
        </div>
      </div>
        ...
        {tasks.map((task) => (
          <div key={task.id} className="grid grid-cols-[280px_1fr] border-b border-[rgba(0,0,0,0.08)] dark:border-[rgba(255,255,255,0.1)] last:border-b-0 hover:bg-[#fafafa] dark:hover:bg-[#2a2a2a]/50 transition-colors">
            <div className="px-6 py-6 border-r border-[rgba(0,0,0,0.08)] dark:border-[rgba(255,255,255,0.1)] bg-white dark:bg-[#1e1e1e]">
              ...
              <div className="text-xs text-[#999] dark:text-[#888]">{task.assignee}</div>
            </div>
            <div className="grid grid-cols-4 relative bg-white dark:bg-[#1e1e1e] py-6">
              {/* Week segments background */}
              {weeks.map((_, index) => (
                <div 
                  key={index} 
                  className="border-l border-[rgba(0,0,0,0.08)] dark:border-[rgba(255,255,255,0.1)] first:border-l-0"
                />
              ))}
              {/* Gantt Bar */}
              <div
    ...
    <div className="bg-white dark:bg-[#1e1e1e] rounded-lg border border-[rgba(0,0,0,0.1)] dark:border-[rgba(255,255,255,0.1)] overflow-hidden">
      ...
      <div className="grid grid-cols-7">
        {calendarDays.map((dayData, index) => (
          ...
          >
            <div className="text-xs text-[#999] dark:text-[#888] mb-2">{dayData.day}</div>
  ...
  const columns = [
    {
      ...
      tasks: [
        { 
          id: 1, 
          title: 'Update API Documentation for Q4 release', 
          hasLink: true,
          assignee: 'MR',
          avatarColor: '#9ca3af',
        },
      ...
      {columns.map((column) => (
        <div key={column.id} className="flex-1">
          <div className="mb-4 flex items-center justify-between">
            ...
            <span className="text-sm text-[#999] dark:text-[#888]">{column.count}</span>
          </div>
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    {task.hasLink && (
                      <Link2 className="size-3.5 text-[#999] dark:text-[#888]" />
                    )}
                    {task.dueDate && (
                      <span 
      ...
      projects: [
        {
          id: 1,
          title: 'MVP Launch',
          description: 'Core features release',
          status: 'on-track',
          color: '#22c55e',
        },
        {
  ...
  return (
    <div className="flex gap-4 h-full p-8">
      {quarters.map((quarter) => (
        <div 
          key={quarter.id} 
          className="flex-1 bg-[#d9d9d9] rounded-2xl p-6"
        >
          <div className="mb-6">
            <h3 className="text-xs font-medium text-[#666] uppercase tracking-wide">{quarter.title}</h3>
          </div>
          <div className="space-y-4">
            {quarter.projects.length > 0 ? (
              quarter.projects.map((project) => (
                ...
                >
                  <h4 className="font-medium text-sm text-[#111] mb-1">{project.title}</h4>
                  {project.description && (
                    <p className="text-xs text-[#999] mb-3">{project.description}</p>
                  )}
                  {project.status && (
                    <div className="flex items-center gap-2">
                      <div 
              ...
              <div className="text-center py-8">
                <p className="text-xs text-[#999]">Planning in progress...</p>
              </div>
            )}
          </div>
        </div>
