    ...
    <div className="p-8 space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="col-span-2 bg-white dark:bg-[#1e1e1e] rounded-lg border border-[rgba(0,0,0,0.05)] dark:border-[rgba(255,255,255,0.1)] p-6">
          <h3 className="text-xs font-medium text-[#999] dark:text-[#888] uppercase tracking-wide mb-5">Pipeline Funnel</h3>
              ...
              <div key={index}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-[#999] dark:text-[#888]">{stage.percentage}%</span>
        ...
        <div className="bg-white dark:bg-[#1e1e1e] rounded-lg border border-[rgba(0,0,0,0.05)] dark:border-[rgba(255,255,255,0.1)] p-6">
          <h3 className="text-xs font-medium text-[#999] dark:text-[#888] uppercase tracking-wide mb-5">Top Sources</h3>
          <div className="space-y-5">
      ...
      {/* Recent Activity */}
      <div className="bg-white dark:bg-[#1e1e1e] rounded-lg border border-[rgba(0,0,0,0.05)] dark:border-[rgba(255,255,255,0.1)] p-6">
        <h3 className="text-xs font-medium text-[#999] dark:text-[#888] uppercase tracking-wide mb-5">Recent Activity</h3>
        <div className="space-y-3">
          {recentActivity.map((activity, index) => (
            ...
            >
              <div className="size-9 rounded-full bg-[#f5f5f5] dark:bg-[#2a2a2a] flex items-center justify-center text-[10px] font-medium text-[#666] dark:text-[#aaa]">
                {activity.initials}
              </div>
              <div className="flex-1">
                <div className="text-sm text-[#111] dark:text-[#e5e5e5] font-medium">{activity.name}</div>
                <div className="text-xs text-[#999] dark:text-[#888]">{activity.action}</div>
              </div>
              <div className="text-[10px] text-[#999] dark:text-[#888]">{activity.time}</div>
            </div>
          ))}
        </div>
      </div>
